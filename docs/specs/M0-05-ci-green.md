...
- Trong:
  ...
  - `.github/dependabot.yml`: pip (backend), npm (frontend), github-actions, docker (backend, frontend) — hằng tuần.
...
| AC-SYS-033 | `.github/dependabot.yml` | đọc cấu hình | có cập nhật `pip` tại `/backend`, `npm` tại `/frontend`, `github-actions` tại `/`, `docker` tại `/backend` và `/frontend`; lịch `weekly` | unit |
