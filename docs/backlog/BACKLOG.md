...
- [R] **M0-02 Scaffold frontend**: Vite React TS strict, `BASE_PATH` cấu hình được (Vite `base`, Router `basename`, API base URL — ADR-014), Tailwind v4 + tokens (UI_GUIDELINES §2), font tự host, ESLint/Prettier/Vitest/Playwright config, trang placeholder dùng token. AC-SYS-002 build ok; AC-SYS-003 không cuộn ngang 360px.
