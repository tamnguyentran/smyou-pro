...
- [ ] **M0-03 Docker & Makefile**: `compose.dev.yml` đầy đủ (hot reload) + `compose.prod.yml`, Dockerfile BE/FE, `make build-prod`/`smoke-prod` chạy được trên Mac M2, `compose.prod.yml` đặt cứng `APP_ENV=production` cho backend (review M0-01), mọi target Makefile chạy được, pre-commit cài được. AC-SYS-004 `make up` → web + api healthy trên Mac M2.
